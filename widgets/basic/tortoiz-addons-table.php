...
class Tortoiz_Addons_Table_Widget extends Widget_Base {
	...
	public function get_title() {
		return __( 'Tortoiz Table', 'tortoiz-addons-ext' );
	}
	...
	public function get_keywords() {
		return [ 'tortoizAddons table', 'data table' ];
	}
	...
	protected function _register_controls() {
		...
		$thead->add_control(
			'header_col_span',
			[
				'label' => __( 'Column Span', 'tortoiz-addons-ext' ),
		...
		$thead->add_control(
			...
			[
				...
				'options' => [
					'left' => __( 'Before', 'tortoiz-addons-ext' ),
					'right' => __( 'After', 'tortoiz-addons-ext' ),
				],
				'default' => 'left',
				'condition' => [
					'header_icon!' => '',
		...
		$tbody->add_control(
			'content_type',
			[
				'label' => __( 'Content Type', 'tortoiz-addons-ext' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'row' => __( 'Row', 'tortoiz-addons-ext' ),
					'cell' => __( 'Cell', 'tortoiz-addons-ext' ),
					'head' => __( 'Head', 'tortoiz-addons-ext' ),
				],
				'default' => 'row',
			]
		);
		$tbody->add_control(
			'row_span',
			[
				'label' => __( 'Row Span', 'tortoiz-addons-ext' ),
				'type' => Controls_Manager::NUMBER,
				'default' => 1,
				'min' => 1,
				'condition' => [
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} table thead th > .tortoiz-addons-icon-right' => 'margin-left: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} table thead th > .tortoiz-addons-icon-left' => 'margin-right: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} table tbody td > .tortoiz-addons-icon-right, {{WRAPPER}} table tbody th > .tortoiz-addons-icon-right' => 'margin-left: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} table tbody td > .tortoiz-addons-icon-left, {{WRAPPER}} table tbody th > .tortoiz-addons-icon-left' => 'margin-right: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_group_control(
	...
	protected function render() {
		...
		?>
		<div class="tortoiz-addons-table">
			<table>
							...
							<?php foreach ($data['header_content'] as $content): ?>
								<th colspan="<?php echo esc_attr( $content['header_col_span'] ); ?>"
									...
									<?php if ( $content['header_icon'] && $content['header_icon_align'] == 'left' ): ?>
										<i class="<?php echo esc_attr($content['header_icon']); ?> tortoiz-addons-icon-left"></i>
									<?php endif; ?>
									<?php printf( '%s', $content['header_text'] ); ?>
									<?php if ( $content['header_icon'] && $content['header_icon_align'] == 'right' ): ?>
										<i class="<?php echo esc_attr($content['header_icon']); ?> tortoiz-addons-icon-right"></i>
									<?php endif; ?>
								</th>
							<?php endforeach; ?>
						</tr>
						...
						<tr class="elementor-repeater-item-<?php echo esc_attr( $key ); ?>">
							<?php foreach ($row as $content) : ?>
								...
								<?php if ( $content['icon'] && $content['icon_align'] == 'left' ): ?>
									<i class="<?php echo esc_attr($content['icon']); ?> tortoiz-addons-icon-left"></i>
								<?php endif; ?>
								<?php printf( '%s', $content['cell_content'] ); ?>
								<?php if ( $content['icon'] && $content['icon_align'] == 'right' ): ?>
									<i class="<?php echo esc_attr($content['icon']); ?> tortoiz-addons-icon-right"></i>
								<?php endif; ?>
								</<?php printf( '%s', $content['type'] ); ?>>
							<?php endforeach; ?>
						</tr>
